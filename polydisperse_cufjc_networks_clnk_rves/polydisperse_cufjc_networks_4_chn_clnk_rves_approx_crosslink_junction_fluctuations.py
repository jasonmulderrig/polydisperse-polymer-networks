...
from src.helpers.clnk_frame_averaging import clnk_frame_avrg_approx

##### This code corresponds to the 20260203A.yaml configuration file.
##### Make sure to set the deformation and topology configuration files
##### to 20260203A.yaml in the config.yaml file before running.
@hydra.main(
        version_base=None,
    ...
    for protocol_indx in range(len(dfrmtn_protocol)):
        ...
        for dfrmtn_step in range(num_dfrmtn_steps):
            ...
            for clnk_indx in range(clnks_num):
                ...
                if (not np.allclose(n_clnk, n_clnk[0]*np.ones_like(n_clnk)) and
                    np.allclose(com_X_hat_clnk, np.zeros(3))):
                    (_, _, gamma_clnk_frame_avrg_approx_so3, _, _, _, _, _, _,

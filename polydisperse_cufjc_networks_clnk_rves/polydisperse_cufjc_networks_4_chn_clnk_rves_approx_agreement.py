...
##### This code corresponds to the 20251204B.yaml configuration file.
##### Make sure to set the deformation and topology configuration files
##### to 20251204B.yaml in the config.yaml file before running.
@hydra.main(
        version_base=None,
                ...
                if not np.allclose(n_clnk, n_clnk[0]*np.ones_like(n_clnk)):
                    ...
                    if k_num == 4:
                        clnk_approx = np.allclose(
                            ...
                            regular_tetrahedral_4_chn_clnk_X_hat_clnk)
                    if clnk_approx:
                        _, _, _, _, _, _, _, _, W_clnk_free_rot_approx = (
                            clnk_free_rot_approx(
                                cfg.deformation.eval_W_clnk_chns,
                                cfg.deformation.eval_W_clnk_y_flucts, True,
                                F, n_clnk, l_n_eq, X_clnk, w_c_func, w_c_args,

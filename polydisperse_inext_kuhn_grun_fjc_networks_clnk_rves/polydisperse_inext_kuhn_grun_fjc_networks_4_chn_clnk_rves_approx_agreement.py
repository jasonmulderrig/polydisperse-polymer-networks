...
from src.helpers.clnk_free_rotation_utils import clnk_free_rot_approx
from src.helpers.clnk_frame_averaging_utils import clnk_frame_avrg_approx
        ...
        for dfrmtn_step in range(num_dfrmtn_steps):
            ...
            for clnk_indx in range(clnks_num):
                ...
                if not np.all(np.equal(n_clnk, n_clnk[0])):
                    ...
                    if clnk_approx:
                        _, _, _, _, _, _, W_clnk_free_rot_approx, _ = (
                    ...
                    if np.allclose(com_X_hat_clnk, np.zeros(3)):
                        _, _, _, _, _, _, _, W_clnk_frame_avrg_approx_so3_quad = (
...
##### This code corresponds to the 20250724B.yaml configuration file.
##### Make sure to set the deformation and topology configuration files
##### to 20250724B.yaml in the config.yaml file before running.
@hydra.main(
        version_base=None,

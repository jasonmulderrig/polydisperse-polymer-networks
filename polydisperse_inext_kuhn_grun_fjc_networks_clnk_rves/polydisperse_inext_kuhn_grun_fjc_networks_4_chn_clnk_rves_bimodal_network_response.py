...
def main(cfg: DictConfig) -> None:
    ...
    # distribution of distinct cross-link structures (with symmetry
    # equivalence)
    n_clnks_filename = filename_prefix + "-n_clnks" + ".dat"
    p_filename = filename_prefix + "-p" + ".dat"
    p_clnks_filename = filename_prefix + "-p_clnks" + ".dat"
    ...
    for protocol_indx in range(len(dfrmtn_protocol)):
        ...
        for dfrmtn_step in range(num_dfrmtn_steps):
            ...
            max_gamma_clnks_free_rot_approx = np.max(
                gamma_clnks_free_rot_approx[protocol_indx][dfrmtn_step])
            max_gamma_clnks_frame_avrg_approx_so3 = np.max(
                gamma_clnks_frame_avrg_approx_so3[protocol_indx][dfrmtn_step])
            print("{}, {}={}, max_gamma_clnks_free_rot_approx={}, max_gamma_clnks_frame_avrg_approx_so3={}".format(dfrmtn_protocol_class_str, dfrmtn_str, dfrmtn_arr[dfrmtn_step], max_gamma_clnks_free_rot_approx, max_gamma_clnks_frame_avrg_approx_so3))

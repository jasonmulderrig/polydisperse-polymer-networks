...
def lexsorted_edges(
        edges: npt.NDArray[np.int64]) -> tuple[npt.NDArray[np.int64], npt.NDArray[np.int64]]:
    ...
    Args:
        edges (npt.NDArray[np.int64]): 2D array with (em, 2) int entries of edges.
    
    Returns:
        tuple[npt.NDArray[np.int64], npt.NDArray[np.int64]]: 2D
...
def edges_to_A_func(edges: npt.NDArray[np.int64]) -> npt.NDArray[np.int64]:
    ...
    Returns:
        npt.NDArray[np.int64]: 2D array with (en, en) int entries of
...
def A_to_edges_func(A: npt.NDArray[np.int64]) -> npt.NDArray[np.int64]:
    ...
    Args:
        A (npt.NDArray[np.int64]): 2D array with (en, en) int entries of the adjacency matrix.
    
    Returns:
        npt.NDArray[np.int64]: 2D array with (em, 2) int entries of
        lexicographically sorted edges.
    
    """
    # Ensure that the adjacency matrix is both square and symmetric
    en = np.shape(A)[0]
...
def edges_and_edges_attr_to_sparse_nghbr_arr_and_sparse_nghbr_attr_arr(
        en: int,
        k_max: int,
        edges: npt.NDArray[np.int64],
        binary_edges_type: npt.NDArray[np.int64] | None = None,
        edges_attr: npt.NDArray[np.float64] | None = None,
        symmtry: bool = True) -> npt.NDArray[np.int64] | tuple[npt.NDArray[np.int64], npt.NDArray[np.float64]] | None:
    """Sparse neighbor array and corresponding sparse neighbor edge
    attributes array converted from edges, binary edge type, and edge
    attributes arrays.

    First and foremost, this function generates a sparse neighbor array
    converted from an edges array. The sparse neighbor array is an
    ...
    between each representation as the sparse neighbor array is
    constructed from the edges array. Also note that the rows in the
    sparse neighbor array are zero-indexed, and referred to as such.
    All zero entries in the sparse neighbor array corresponds the
    absence of an edge.

    The sparse neighbor array is able to naturally represent a binary
    ...
    the sparse neighbor array, an entry "val" in the ith row
    ...
    neighbor array containing corresponding edge attributes. To execute
    this functionality, an edge attributes array (corresponding to the
    edges array) can be optionally provided. Each sparse neighbor edge
    attributes array entry thus stores the attribute of the edge in the
    corresponding sparse neighbor array entry.

    By default, this function generates a symmetric sparse neighbor
    array. However, if called for, a sparse neighbor array that
    ...
    sparse neighbor array is initialized by addressing each edge
    provided in the edges array, without any additional considerations.
    If a sparse neighbor edge attributes array is also called for, then
    ...
    Args:
        ...
        edges (npt.NDArray[np.int64]): 2D array with (em, 2) int entries of edges.
        binary_edges_type (npt.NDArray[np.int64] | None): 1D array with em int entries of binary edge type labels. Labels can be either 1 or 2.
        edges_attr (npt.NDArray[np.float64] | None): 1D array with em float entries of edge attributes.
        symmtry (bool): Boolean flag to create either a symmetric (if True) or an upper-triangular (if False) sparse neighbor array. If called for, a corresponding symmetric or upper-triangular sparse neighbor edge attributes array will also be created.
    
    Returns:
        npt.NDArray[np.int64] | tuple[npt.NDArray[np.int64], npt.NDArray[np.float64]] | None:
        Sparse neighbor array; Sparse neighbor array and corresponding
        sparse neighbor edge attributes array.
    ...
    # Initialize the sparse neighbor array. If provided, also
    # initialize the sparse neighbor edge attributes array.
    sparse_nghbr_arr = np.zeros((en, k_max), dtype=int)
    if edges_attr is not None: sparse_nghbr_attr_arr = np.zeros((en, k_max))

    # Directly enter each edge from the edges array in the sparse
    # neighbor array in order to create the default upper-triangular 
    # sparse neighbor array
    for edge in range(em):
        # Node numbers
        ...
        # sparse neighbor array
        sparse_nghbr_node = core_node_1 + 1

        # Determine appropriate column index 
        k_indx = np.where(sparse_nghbr_arr[core_node_0]==0)[0][0]

        # Input sparse neighbor array entry. If provided, follow
        # convention to account for binary edge types
        if binary_edges_type is not None:
            sparse_nghbr_arr[core_node_0, k_indx] = int(
                binary_edges_type_copy[edge] * sparse_nghbr_node)
        else: sparse_nghbr_arr[core_node_0, k_indx] = sparse_nghbr_node

        # If provided, input the edge attribute to the sparse neighbor
        # edge attributes array
        if edges_attr is not None:
            sparse_nghbr_attr_arr[core_node_0, k_indx] = edges_attr_copy[edge]
        
        # Each primary loop is entered in the sparse neighbor array
        ...
        if core_node_0 == core_node_1:
            ...
            # neighbor edge attributes array
            if edges_attr is not None:
                sparse_nghbr_attr_arr[core_node_0, k_indx] = edges_attr_copy[edge]
    
    # If called for, symmetrize the sparse neighbor array
    if symmtry:
        for edge in range(em):
            # Node numbers
            core_node_0 = int(edges_copy[edge, 0])
            ...
            if core_node_0 == core_node_1: continue
                ...
                # entry in the sparse neighbor array
                sparse_nghbr_node = core_node_0 + 1

                # Determine appropriate column index 
                k_indx = np.where(sparse_nghbr_arr[core_node_1]==0)[0][0]

                # Input sparse neighbor array entry. If provided,
                # follow convention to account for binary edge types
                if binary_edges_type is not None:
                    sparse_nghbr_arr[core_node_1, k_indx] = int(
                        binary_edges_type_copy[edge] * sparse_nghbr_node)
                else: sparse_nghbr_arr[core_node_1, k_indx] = sparse_nghbr_node

                # If provided, input the edge attribute to the sparse
                # neighbor edge attributes array
                if edges_attr is not None:
                    sparse_nghbr_attr_arr[core_node_1, k_indx] = edges_attr_copy[edge]
    
    # Sort the values of each row of the sparse neighbor array. If
    # provided, correspondingly sort the values of the sparse neighbor
    # edge attributes array 
    sort_indcs = np.argsort(sparse_nghbr_arr, axis=1)
    sparse_nghbr_arr = np.take_along_axis(sparse_nghbr_arr, sort_indcs, axis=1)
    if edges_attr is not None:
        sparse_nghbr_attr_arr = np.take_along_axis(
            sparse_nghbr_attr_arr, sort_indcs, axis=1)
        return sparse_nghbr_arr, sparse_nghbr_attr_arr
    else: return sparse_nghbr_arr

def sparse_nghbr_arr_and_sparse_nghbr_attr_arr_to_edges_and_edges_attr(
        sparse_nghbr_arr: npt.NDArray[np.int64],
        return_binary_edges_type: bool = False,
        sparse_nghbr_attr_arr: npt.NDArray[np.float64] | None = None) -> npt.NDArray[np.int64] | tuple[npt.NDArray[np.int64], npt.NDArray[np.int64]] | tuple[npt.NDArray[np.int64], npt.NDArray[np.int64], npt.NDArray[np.float64]] | None:
    """Edges, binary edge types, and edge attributes arrays converted
    from sparse neighbor array and corresponding sparse neighbor edge
    attributes array.
    
    First and foremost, this function generates an edges array converted
    from a sparse neighbor array. If called for, this function is also
    ...
    functionality, a sparse neighbor edge attributes array
    (corresponding to the sparse neighbor array array) can be
    optionally provided.

    Args:
        sparse_nghbr_arr (npt.NDArray[np.int64]): Sparse neighbor array.
        return_binary_edges_type (bool): Boolean flag to return the binary edge types array (if True) or not (if False).
        sparse_nghbr_attr_arr (npt.NDArray[np.float64] | None): Sparse neighbor edge attributes array.
    
    Returns:
        npt.NDArray[np.int64] | tuple[npt.NDArray[np.int64], npt.NDArray[np.int64]] | tuple[npt.NDArray[np.int64], npt.NDArray[np.int64], npt.NDArray[np.float64]] | None:
    ...
    """
    # Make copies of the supplied arrays
    sparse_nghbr_arr_copy = np.atleast_2d(sparse_nghbr_arr.copy())
    if sparse_nghbr_attr_arr is not None:
        sparse_nghbr_attr_arr_copy = np.atleast_2d(sparse_nghbr_attr_arr.copy())

    # Confirm that the nodes in the provided network are within the
    # specified number of nodes, as dictated by the number of rows in
    # the provided sparse neighbor array
    sparse_nghbr_arr_en = np.max(np.abs(sparse_nghbr_arr_copy))
    en = np.shape(sparse_nghbr_arr_copy)[0]
    if sparse_nghbr_arr_en > en:
        error_str = (
            "The largest node number in the provided sparse neighbor "
    ...
    # If provided, confirm that the sparse neighbor edge attributes
    # array has the same shape as the sparse neighbor array
    if sparse_nghbr_attr_arr is not None:
        if np.shape(sparse_nghbr_arr_copy) != np.shape(sparse_nghbr_attr_arr_copy):
            error_str = (
                "The shape of the sparse neighbor edge attributes is "
                + "not the same shape as the sparse neighbor array."
            )
            raise ValueError(error_str)

    # Determine total number of edges in the sparse neighbor array
    em = 0
    for core_node_0 in range(en):
        # Node is connected to at least one other node
        if np.count_nonzero(sparse_nghbr_arr_copy[core_node_0]) > 0:
            # Evaluate primary loops. Gather indices of connection sites
            # occupied by primary loops.
            primary_loop_k_indcs = (
                np.where(np.logical_or(sparse_nghbr_arr_copy[core_node_0]==core_node_0+1, sparse_nghbr_arr_copy[core_node_0]==-1*(core_node_0+1)))[0]
            )
            ...
            if num_primary_loop_k_indcs > 0:
                ...
                sparse_nghbr_arr_copy[core_node_0, primary_loop_k_indcs] = 0
            ...
            if np.count_nonzero(sparse_nghbr_arr_copy[core_node_0]) > 0:
                ...
                for k_indx in np.nditer(np.nonzero(sparse_nghbr_arr_copy[core_node_0])[0]):
                    k_indx = int(k_indx)
                    sparse_nghbr_node = int(
                        sparse_nghbr_arr_copy[core_node_0, k_indx])

                    # Adjust neighbor node number from sparse neighbor
                    # array for entry in the edges array
                    if sparse_nghbr_node > 0:
                        core_node_1 = sparse_nghbr_node - 1
                    else: core_node_1 = -1 * sparse_nghbr_node - 1

                    # Remove edge
                    sparse_nghbr_arr_copy[core_node_0, k_indx] = 0

                    # Remove symmetric duplicate edge entry, if it
                    # exists
                    if sparse_nghbr_node > 0:
                        symmtrc_sparse_nghbr_node = core_node_0 + 1
                    else: symmtrc_sparse_nghbr_node = -1 * (core_node_0+1)
                    symmtrc_k_indx_arr = (
                        np.where(sparse_nghbr_arr_copy[core_node_1]==symmtrc_sparse_nghbr_node)[0]
                    )
                    if np.size(symmtrc_k_indx_arr) > 0:
                        symmtrc_k_indx = symmtrc_k_indx_arr[0]
                        sparse_nghbr_arr_copy[core_node_1, symmtrc_k_indx] = 0
                    em += 1
    
    # Make a new copy of the sparse neighbor array
    sparse_nghbr_arr_copy = sparse_nghbr_arr.copy()
    ...
    if sparse_nghbr_attr_arr is not None: edges_attr = np.empty(em)

    # Directly enter each edge from the sparse neighbor array in the 
    # edges array
    edge = 0
    for core_node_0 in range(en):
        # Node is connected to at least one other node
        if np.count_nonzero(sparse_nghbr_arr_copy[core_node_0]) > 0:
            ...
            if num_primary_loop_k_indcs > 0:
                ...
                # neighbor array twice, and each such pairing will
                ...
                for indx in range(0, num_primary_loop_k_indcs, 2):
                    ...
                    if return_binary_edges_type:
                        if sparse_nghbr_arr_copy[core_node_0, primary_loop_k_indcs[indx]] > 0:
                    ...
                    if sparse_nghbr_attr_arr is not None:
                        edges_attr[edge] = (
                            sparse_nghbr_attr_arr_copy[core_node_0, primary_loop_k_indcs[indx]]
                        )
                    edge += 1
                ...
                if sparse_nghbr_attr_arr is not None:
                    sparse_nghbr_attr_arr_copy[core_node_0, primary_loop_k_indcs] = 0.
            ...
            if np.count_nonzero(sparse_nghbr_arr_copy[core_node_0]) > 0:
                ...
                for k_indx in np.nditer(np.nonzero(sparse_nghbr_arr_copy[core_node_0])[0]):
                    ...
                    # If called for, store the binary edge type
                    if return_binary_edges_type:
                        if sparse_nghbr_node > 0: binary_edges_type[edge] = 1
                        else: binary_edges_type[edge] = 2
                    
                    # If provided, store the edge attribute
                    if sparse_nghbr_attr_arr is not None:
                        edges_attr[edge] = (
                            sparse_nghbr_attr_arr_copy[core_node_0, k_indx]
                        )
                    ...
                    # If provided, remove the edge attribute
                    if sparse_nghbr_attr_arr is not None:
                        sparse_nghbr_attr_arr_copy[core_node_0, k_indx] = 0.
                    ...
                    if np.size(symmtrc_k_indx_arr) > 0:
                        ...
                        # If provided, remove the edge attribute
                        if sparse_nghbr_attr_arr is not None:
                            sparse_nghbr_attr_arr_copy[core_node_1, symmtrc_k_indx] = (
                                0.
                            )
                    edge += 1
    ...
    if return_binary_edges_type and sparse_nghbr_attr_arr is not None:
        return edges, binary_edges_type, edges_attr
    elif return_binary_edges_type and sparse_nghbr_attr_arr is None:
        return edges, binary_edges_type
    elif not return_binary_edges_type and sparse_nghbr_attr_arr is not None:
        return edges, edges_attr
    else: return edges

def A_largest_connected_component(
        A: npt.NDArray[np.int64]) -> npt.NDArray[np.int64]:
...
def sparse_nghbr_arr_largest_connected_component(
        en: int,
        k_max: int,
        edges: npt.NDArray[np.int64],
        binary_edges_type: npt.NDArray[np.int64] | None = None,
        edges_attr: npt.NDArray[np.float64] | None = None) -> npt.NDArray[np.int64] | tuple[npt.NDArray[np.int64], npt.NDArray[np.int64]] | tuple[npt.NDArray[np.int64], npt.NDArray[np.int64], npt.NDArray[np.float64]] | None:
    """Largest/maximum connected component extraction via the sparse
    neighbor array.

    This function isolates and returns the largest/maximum connected
    component via analyzing the sparse neighbor array.
    ...
    """
    # Gather symmetric sparse neighbor array, and if called for, the
    # sparse neighbor edge attributes array
    if edges_attr is not None:
        sparse_nghbr_arr, sparse_nghbr_attr_arr = (
    ...
    # Return the absolute value of all sparse neighbor array entries,
    # and correspondingly re-sort the array entries
    abs_sparse_nghbr_arr = np.abs(sparse_nghbr_arr)
    abs_sparse_nghbr_arr = np.take_along_axis(
        abs_sparse_nghbr_arr, np.argsort(abs_sparse_nghbr_arr, axis=1), axis=1)
        ...
        if not visited[start_node]:
            ...
            while len(queue) > 0:
                ...
                if not visited[node]:
                    visited[node] = True
                    component.append(node)
                    if np.count_nonzero(abs_sparse_nghbr_arr[node]) > 0:
                        for k_indx in np.nditer(np.nonzero(abs_sparse_nghbr_arr[node])[0]):
                            k_indx = int(k_indx)
                            nghbr = int(abs_sparse_nghbr_arr[node, k_indx]-1)
                            if not visited[nghbr]: queue.append(nghbr)
    ...
    # Extract the sparse neighbor array of the largest connected
    # component. If called for, extract the sparse neighbor edge
    # attributes array of the largest connected component.
    sparse_nghbr_arr_lcc = np.zeros_like(sparse_nghbr_arr)
    sparse_nghbr_arr_lcc[lcc_nodes] = sparse_nghbr_arr[lcc_nodes]
    if edges_attr is not None:
        sparse_nghbr_attr_arr_lcc = np.zeros_like(sparse_nghbr_attr_arr)
        sparse_nghbr_attr_arr_lcc[lcc_nodes] = sparse_nghbr_attr_arr[lcc_nodes]
    ...
    if binary_edges_type is not None and edges_attr is not None:
        return (
            sparse_nghbr_arr_and_sparse_nghbr_attr_arr_to_edges_and_edges_attr(
                sparse_nghbr_arr_lcc, return_binary_edges_type=True,
                sparse_nghbr_attr_arr=sparse_nghbr_attr_arr_lcc)
        )
    elif binary_edges_type is not None and edges_attr is None:
        return (
            sparse_nghbr_arr_and_sparse_nghbr_attr_arr_to_edges_and_edges_attr(
                sparse_nghbr_arr_lcc, return_binary_edges_type=True,
                sparse_nghbr_attr_arr=None)
        )
    elif binary_edges_type is None and edges_attr is not None:
        return (
            sparse_nghbr_arr_and_sparse_nghbr_attr_arr_to_edges_and_edges_attr(
                sparse_nghbr_arr_lcc, return_binary_edges_type=False,
                sparse_nghbr_attr_arr=sparse_nghbr_attr_arr_lcc)
        )
    else:
        return (

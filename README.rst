...
This work is built upon the Python scientific computing stack, e.g., `Numpy <https://numpy.org/>`_, `Scipy <https://scipy.org/>`_, `Matplotlib <https://matplotlib.org/>`_, and `scikit-learn <https://scikit-learn.org/stable/>`_. Parameter configuration management is handled via the `Hydra <https://hydra.cc/>`_ package.
...
Calculus- and linear algebra-based derivations that directly yield various functions in the ``src/helpers`` directory are included in the ``mathematica_outputs`` directory.
...
\Jason Mulderrig, Michael Buche, and Matthew Grasinger, Polydisperse polymer networks with irregular topologies II: Investigating the synthesis-topology-property relationship of bimodal PDMS networks and vulcanized rubbers, In preparation.

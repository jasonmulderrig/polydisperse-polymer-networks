            ...
            for clnk_indx in range(clnks_num):
                ...
                if (np.all(np.equal(n_clnk, n_clnk[0])) and
                    ...
                    if monodisperse_clnk:
                        _, y_clnk_star, _, _, W_clnk_star, _ = (
                            monodisperse_clnk_free_rot(
                    ...
                    if k_num == 3:
                        ...
                        monodisperse_clnk = np.allclose(
                            X_hat_clnk, cube_8_chn_clnk_X_hat_clnk)
                    if monodisperse_clnk:
                        (y_clnk_star_frame_avrg_so3, _, _, _, _, _, _,
                ...
                # Evaluate the cross-link structure deformation
                    ...
                    (omega_clnk_free_rot, _, y_clnk_free_rot, _, _,
                     W_clnk_free_rot, _) = clnk_free_rot_cnstrnd_mnmztn(
...
##### This code corresponds to the 20250724E.yaml configuration file.
##### Make sure to set the deformation and topology configuration files
##### to 20250724E.yaml in the config.yaml file before running.
@hydra.main(
        version_base=None,

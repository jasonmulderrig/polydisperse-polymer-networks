...
from src.helpers.clnk_frame_averaging import clnk_frame_avrg_cnstrnd_mnmztn

##### This code corresponds to the 20251204D.yaml configuration file.
##### Make sure to set the deformation and topology configuration files
##### to 20251204D.yaml in the config.yaml file before running.
@hydra.main(
        version_base=None,
        ...
        for dfrmtn_step in range(num_dfrmtn_steps):
            ...
            for clnk_indx in range(clnks_num):
                ...
                (omega_clnk_free_rot, _, y_clnk_free_rot, y_clnk_norm_free_rot,
                 _, _, _, W_clnk_free_rot) = clnk_free_rot_cnstrnd_mnmztn(
                ...
                (y_clnk_frame_avrg_so3, _, _, _, _, _, _,
                 y_clnk_norm_frame_avrg_so3_quad, _, _, _,
                 W_clnk_frame_avrg_so3_quad) = clnk_frame_avrg_cnstrnd_mnmztn(
                    cfg.deformation.eval_W_clnk_chns,
                    cfg.deformation.eval_W_clnk_y_flucts,
                    cfg.deformation.cnstrnd_mnmztn_scope,
                    cfg.deformation.cnstrnd_mnmztn_method, rng, F, n_clnk,
                    cfg.topology.b, X_clnk, so3_quad, sph_quad_symmtry,

    ...
    for protocol_indx in range(dfrmtn_protocol_num):
        ...
        for set_indx in range(geo_isomrphc_sets_num):
            ...
            for dfrmtn_step in range(np.shape(dfrmtn_arr)[0]):
                ...
                # Evaluate the cross-link structure deformation via the
                # free rotation model for monodisperse cross-link
                # structures
                if np.allclose(n_clnks_geo_isomrphc_sets[set_indx][0], n_clnks_geo_isomrphc_sets[set_indx][0, 0]):
                    (_, _, _, _, W_clnk_chns_star_free_rot,
                     W_clnk_y_flucts_star_free_rot) = monodisperse_clnk_free_rot(
                        cfg.deformation.eval_W_clnk_y_flucts, F, Lmbda,
                        n_clnks_geo_isomrphc_sets[set_indx][0],
                        b_clnks_geo_isomrphc_sets[set_indx][0],
                        X_clnks_geo_isomrphc_sets[set_indx][0],
                        y_clnks_init_geo_isomrphc_sets[set_indx][0],
                        gamma_clnks_init_geo_isomrphc_sets[set_indx][0],
                        w_c_func_clnks_geo_isomrphc_sets[set_indx][0],
                        w_c_args_clnks_geo_isomrphc_sets[set_indx][0],
                        d2w_c__dy_clnk_dy_clnk_func_clnks_geo_isomrphc_sets[set_indx][0],
                        d2w_c__dy_clnk_dy_clnk_args_clnks_geo_isomrphc_sets[set_indx][0],
                        w_c_dfrmtn_func_clnks_geo_isomrphc_sets[set_indx][0],
                        w_c_dfrmtn_args_clnks_geo_isomrphc_sets[set_indx][0])
                    W_clnks_geo_isomrphc_sets_free_rot_approx[protocol_indx][set_indx][0, dfrmtn_step] = (
                        W_clnk_chns_star_free_rot + W_clnk_y_flucts_star_free_rot
                    )
                
                # Evaluate the deformation of each geometrically
                # isomorphic set of cross-link structures via the free
                # rotation approximation
                else:
                    (_, _, _, _, _,
                     W_clnk_chns_clnks_geo_isomrphc_set_free_rot_approx,
                     W_clnk_y_flucts_clnks_geo_isomrphc_set_free_rot_approx, _,
                     _, _, _, _, _, _) = clnk_free_rot_approx(
